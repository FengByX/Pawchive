...
class DownloadWorker(
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        // 标记为运行中
        historyManager.updateStatus(url, DownloadStatus.RUNNING, progress = 0)
        ...
        try {
            // 2) 复用 ApiClient.sharedOkHttpClient：自动注入 cf_clearance / User-Agent，403 透明重试
            ...
            val (os, mediaUri) = DownloadRepository.openDownloadStream(context, target)
            outputStream = os
            ...
            while (true) {
                ...
                if (contentLength > 0) {
                    ...
                    if (percent - lastReported >= PROGRESS_STEP || percent == 100) {
                        ...
                        historyManager.updateStatus(url, DownloadStatus.RUNNING, progress = percent, fileSize = contentLength)
                    }
                }
            }
            ...
            DownloadRepository.finalizeDownload(context, mediaUri)
            ...
            if (isStopped) {
                ...
                historyManager.updateStatus(url, DownloadStatus.CANCELLED)
                Result.failure()
            } else {
                val friendly = ErrorMessageHelper.getFriendlyMessage(context, e)
                historyManager.updateStatus(url, DownloadStatus.FAILED, errorMessage = friendly)
                if (hasNotifyPermission) {
