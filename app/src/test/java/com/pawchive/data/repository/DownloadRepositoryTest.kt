...
class DownloadRepositoryTest {
    ...
    fun setup() {
        // 清空 SAF 配置，保证测试隔离
        SettingsManager.getInstance(context).setDownloadTreeUri(null, "")
    }
    ...
    fun `openDownloadStream without SAF config falls back to MediaStore`() {
        ...
        try {
            val (stream, uri) = DownloadRepository.openDownloadStream(context, target)
    ...
    fun `openDownloadStream with invalid SAF uri falls back to MediaStore`() {
        ...
        SettingsManager.getInstance(context).setDownloadTreeUri(invalidUri, "invalid")
    ...
    fun `finalizeDownload with null uri is no-op`() {
        ...
        DownloadRepository.finalizeDownload(context, null)
    }
    ...
    fun `finalizeDownload with non-null uri does not throw`() {
        ...
        // 不应抛异常
        DownloadRepository.finalizeDownload(context, fakeUri)
    }
    ...
    fun `SettingsManager SAF config is read by DownloadRepository`() {
        ...
        SettingsManager.getInstance(context).setDownloadTreeUri(safUri, "Pawchive")

        val savedUri = SettingsManager.getInstance(context).getDownloadTreeUri()
