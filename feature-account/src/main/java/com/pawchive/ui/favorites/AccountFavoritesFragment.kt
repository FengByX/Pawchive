...
class AccountFavoritesFragment : Fragment() {
     ...
     *
     * 覆盖场景：
     * 1. Activity 重建 + Cloudflare 过盾时序：onViewCreated 时过盾未完成导致 loadCreators 失败，
     *    loadedCreators 仍为空；onResume 时过盾已就绪，ensureCurrentTabLoaded 补触发
     * 2. ViewPager2 切回收藏 Fragment（实例保留、View 未重建）：onViewCreated 不重调，
     *    若 ViewModel 新建过且加载失败，loadedCreators 为空；onResume 补触发
     *
     * 幂等：ViewModel.ensureCurrentTabLoaded 内部会检查 isLoading 和 loaded 状态，
     *      数据已在加载或已加载时直接返回，不会重复请求。
     */
    override fun onResume() {
        super.onResume()
        if (authRepository.isLoggedIn()) {
            viewModel.ensureCurrentTabLoaded()
        }
    }
    ...
    private fun setupAdapters() {
        ...
        // 关键修复：初始 adapter 必须与 currentTab 一致。
        // 否则 savedInstanceState != null（Activity 重建/进程恢复）时，
        // onViewCreated 走 setScrollPosition 分支不会触发 onTabSelected 回调，
        // adapter 停留在默认 postAdapter，导致创作者 Tab 数据加载了却显示为空。
